...
class HeartBeatScreen extends StatefulWidget {
  @override
  HeartBeatScreenView createState() {
    return HeartBeatScreenView();
  }
}

class HeartBeatScreenView extends State<HeartBeatScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          if ((_bpm != 0) && (_bpm > 30 && _bpm < 150)) {
            savingHeartBeatHistory({
              'bpm': _bpm,
              // 'time': DateFormat('yyyy-MM-dd – kk:mm').format(DateTime.now())
              'time': DateTime.now()
            });
            // SavedPreference.saveBPM({
            //   'bpm': _bpm,
            //   'time': DateFormat('yyyy-MM-dd – kk:mm').format(DateTime.now())
            // });
            Navigator.pop(context);
            snackBar(context, "BPM SAVED into History...");
          } else {
            snackBar(context, "Take Readig First");
          }
        },
